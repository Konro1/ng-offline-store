...
export class UserEffects {
    ...
    @Effect() getUsers$ = this.update$.ofType(UserActions.GET_USER_REQUEST)
        .switchMap(() => {
            return this.userService.getUsers()
        })
        .map(users => this.userActions.loadUserSuccess(users));
    ...
    @Effect() addUser$ = this.update$
        ...
        .mergeMap(action => {
            return this.userService.addUser(action.payload)
                ...
                .catch(error => {
                    const unsyncedAction = this.userActions.addUser(action.payload);
